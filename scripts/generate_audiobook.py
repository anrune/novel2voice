...
def auto_parse_to_segments(novel_text, character_voice_map=None, tts_backend="edge", single_voice=None):
    """
    自动解析小说文本为 segments.json 格式
    character_voice_map: {角色名: voice_id} 可选的手动音色覆盖
    tts_backend: "edge" 或 "mimo"，影响默认音色
    single_voice: 单角色模式，全文（旁白 + 所有角色）使用同一音色
    """
    if not HAS_PARSER:
    ...
    for name, char in characters.items():
        ...
        if char.voice_style:
            ...
            styles[name] = ""
    # v2.13: narrator 音色默认使用后端默认，但允许 --voices 用 "narrator"/"旁白" 覆盖
    narrator_override = None
    if character_voice_map:
        for _k in ("narrator", "旁白"):
            if _k in character_voice_map:
                narrator_override = character_voice_map[_k]
                break
    voices["narrator"] = narrator_override or narrator_voice

    # v2.14: 单角色模式 —— 全文（旁白 + 所有角色）使用同一音色，并清空风格避免干扰
    if single_voice:
        for _k in voices:
            voices[_k] = single_voice
        for _k in styles:
            styles[_k] = ""
...
def main():
    ...
    parser.add_argument("--single-voice", type=str, default=None, help="单角色模式：全文（旁白+角色）使用同一音色")
    ...
    if args.auto:
        ...
        data = auto_parse_to_segments(novel_text, voice_map, tts_backend=tts_backend, single_voice=args.single_voice)
        data["title"] = os.path.splitext(os.path.basename(args.input))[0]
    else:
        # 传统 segments.json 模式
        with open(args.input, 'r', encoding='utf-8') as f:
